...
const COMPANY_INFO = {
  ...
  bankingDetails: {
    companyName: 'Smart Universe Comm and IT',
...
const drawPartyBlock = (
  ...
  y: number,
  width: number,
  details: Counterparty | Record<string, string | undefined>
) => {
  const lines = Object.values(details).filter(Boolean) as string[];
...
function drawHeader(doc: jsPDF, invoice: InvoicePdfData, arabicHeaderImage?: string) {
  const pageWidth = doc.internal.pageSize.getWidth();
  ...
  const companyNameLines = doc.splitTextToSize(COMPANY_INFO.legalName, leftTextWidth).slice(0, 3);
  doc.text(companyNameLines, leftTextX, 18);

  doc.setFont('helvetica', 'normal');
  doc.setFontSize(8.7);
  doc.setTextColor(55, 65, 81);
  const addressLines = doc.splitTextToSize(COMPANY_INFO.address, leftTextWidth).slice(0, 3);
  const companyBlockBottomY = 18 + companyNameLines.length * 5.4;
  ...
  doc.text('INVOICE', PAGE_MARGIN_X, 70);

  doc.setDrawColor(220, 226, 237);
  doc.roundedRect(pageWidth - 59, 47, 45, 22, 4, 4);
  doc.setFontSize(8.8);
  doc.setTextColor(37, 64, 173);
  doc.text('Invoice #', pageWidth - 55, 53);
  doc.text('Date', pageWidth - 55, 59);
  doc.text('Due Date', pageWidth - 55, 65);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(17, 24, 39);
  doc.setFontSize(8.2);
  doc.text(invoice.invoiceNumber, pageWidth - 16.5, 53, { align: 'right' });
  doc.text(format(invoice.createdAt, 'dd MMM yyyy'), pageWidth - 16.5, 59, { align: 'right' });
  doc.text(format(invoice.dueDate, 'dd MMM yyyy'), pageWidth - 16.5, 65, { align: 'right' });
}

export async function generateInvoicePDF(invoice: InvoicePdfData, customer: Counterparty) {
  ...
  const normalizedLineItems = invoice.lineItems && invoice.lineItems.length > 0
    ? invoice.lineItems
    : [{
        id: 'fallback',
        description: invoice.notes?.trim() || 'Invoice Amount',
        quantity: 1,
  ...
  const leftHeight = drawPartyBlock(doc, 'Bill To', PAGE_MARGIN_X, PARTY_BLOCK_Y, 88, {
    name: customer.name ? `Name: ${customer.name}` : undefined,
    company: customer.company ? `Company: ${customer.company}` : undefined,
    address: customer.address ? `Address: ${customer.address}` : undefined,
    phone: customer.phone ? `Phone: ${customer.phone}` : undefined,
    email: customer.email ? `Email: ${customer.email}` : undefined,
  });

  const rightHeight = drawPartyBlock(doc, 'Invoice Details', 108, PARTY_BLOCK_Y, 88, {
    status: `Status: ${String(invoice.status || 'draft').toUpperCase()}`,
    paymentStatus: `Payment Status: ${String(invoice.paymentStatus || 'unpaid').replace(/_/g, ' ').toUpperCase()}`,
    currency: 'Currency: Saudi Riyal (SAR)',
    project: invoice.projectId ? `Project Ref: ${invoice.projectId}` : undefined,
    quotation: invoice.quotationId ? `Quotation Ref: ${invoice.quotationId}` : undefined,
    paidDate: invoice.paidDate ? `Paid Date: ${format(invoice.paidDate, 'dd MMM yyyy')}` : undefined,
  });
  ...
  if (invoice.customerId) {
    doc.text(`Customer ID: ${invoice.customerId}`, PAGE_MARGIN_X, metaY);
    metaY += 6;
  }
  ...
  autoTable(doc, {
    ...
    body: normalizedLineItems.map((item, index) => [
      String(index + 1),
      item.description,
    ...
    didDrawCell: (data) => {
      ...
      const item = normalizedLineItems[data.row.index];
      const amount = data.column.index === 3 ? Number(item?.unitPrice || 0) : Number(item?.total || 0);
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(9);
      doc.setTextColor(31, 41, 55);
      drawCurrencyValue(doc, formatCurrencyAmount(amount), data.cell.x + data.cell.width - 2.2, data.cell.y + data.cell.height / 2 + 1.3, {
        align: 'right',
  ...
  const bankingX = PAGE_MARGIN_X;
  const bankingWidth = 94;
  const summaryX = 116;

  if (summaryY + 69 > pageHeight - FOOTER_HEIGHT - 6) {
    doc.addPage();
    drawHeader(doc, invoice, arabicHeaderImage);
    drawFooter(doc);
  ...
  doc.setDrawColor(220, 226, 237);
  doc.roundedRect(summaryX, summaryY, 80, 28, 3, 3);
  doc.setFont('helvetica', 'bold');
  doc.setFontSize(10);
  doc.setTextColor(17, 24, 39);
  doc.text('Subtotal in SAR', summaryX + 4, summaryY + 7);
  drawCurrencyValue(doc, formatCurrencyAmount(subtotal), summaryX + 76, summaryY + 7, {
    align: 'right',
    iconDataUrl: riyalSymbolImage,
    iconW: 3.2,
    iconH: 3.2,
  ...
  doc.text('VAT 15%', summaryX + 4, summaryY + 15);
  drawCurrencyValue(doc, formatCurrencyAmount(vatAmount), summaryX + 76, summaryY + 15, {
    align: 'right',
    iconDataUrl: riyalSymbolImage,
    iconW: 3.2,
    iconH: 3.2,
  ...
  doc.setTextColor(30, 64, 175);
  doc.text('Total Price with VAT in SAR', summaryX + 4, summaryY + 23);
  drawCurrencyValue(doc, formatCurrencyAmount(total), summaryX + 76, summaryY + 23, {
    align: 'right',
    iconDataUrl: riyalSymbolImage,
    iconW: 3.2,
    iconH: 3.2,
  ...
  doc.setFontSize(10);
  doc.setTextColor(30, 64, 175);
  doc.text('Banking Details', bankingX + 4, summaryY + 7);
  doc.setFontSize(8.8);
  doc.setTextColor(55, 65, 81);
  doc.text('Company Name:', bankingX + 4, summaryY + 14);
  doc.text('Bank:', bankingX + 4, summaryY + 20);
  doc.text('IBAN:', bankingX + 4, summaryY + 26);
  doc.text('Account Number:', bankingX + 4, summaryY + 32);
  doc.setFont('helvetica', 'normal');
  doc.text(COMPANY_INFO.bankingDetails.companyName, bankingX + 31, summaryY + 14);
  doc.text(COMPANY_INFO.bankingDetails.bankName, bankingX + 31, summaryY + 20);
  doc.text(COMPANY_INFO.bankingDetails.iban, bankingX + 31, summaryY + 26);
  doc.text(COMPANY_INFO.bankingDetails.accountNumber, bankingX + 31, summaryY + 32);

  let notesY = summaryY + 43;
  const notes = invoice.notes?.trim() ? [invoice.notes.trim()] : [];

  if (notes.length > 0) {
    const wrappedNotes = notes.map((line) => doc.splitTextToSize(line, doc.internal.pageSize.getWidth() - PAGE_MARGIN_X * 2));
